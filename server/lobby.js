...
function leaveLobby(room, id) {
    ...
    if ((id == lobbys[room].player1 || id == lobbys[room].player2) && lobbys[room].players.length > 1) {
    ...
    asArray = asArray.filter(([key, val]) => {
        return (key != id && !(key == lobbys[room].player1 || key == lobbys[room].player2));
    });

    lobbys[room].spectators = Object.fromEntries(asArray);
    io.sockets.to(room).emit('spectators', Object.values(lobbys[room].spectators));
};
